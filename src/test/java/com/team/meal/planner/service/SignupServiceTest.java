...
class SignupServiceTest {
    ...
    void setUp() {
        ...
        signupService = new SignupService(signupRepository, mealRepository, personRepository, entityManager);
    }

    @Test
    void createSignup_idempotent_existingSignupReturned() {
        Meal meal = new Meal(); meal.setDate(LocalDateTime.now().plusDays(1));
        meal.setMaxAttendees(5);
        meal.setTitle("T1");
        meal.setCuisine("Indian");

        Person person = new Person(); person.setName("Alice");
    ...
    void createSignup_conflict_alreadySignedSameDay() {
        Meal meal = new Meal(); meal.setDate(LocalDateTime.now().plusDays(2));
        Person person = new Person(); person.setName("Bob");
        ...
        when(signupRepository.findByPersonIdAndMealDateRange(eq(20L), any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(List.of(new Signup()));
    ...
    void createSignup_conflict_mealFull() {
        Meal meal = new Meal(); meal.setDate(LocalDateTime.now().plusDays(3)); meal.setMaxAttendees(2);
        Person person = new Person(); person.setName("Carol");

        when(mealRepository.findById(11L)).thenReturn(Optional.of(meal));
        ...
        when(signupRepository.findByPersonIdAndMealDateRange(eq(21L), any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(List.of());
    ...
    void createSignup_conflict_dietaryMismatch() {
        Meal meal = new Meal(); meal.setDate(LocalDateTime.now().plusDays(4));
        meal.setTags(new HashSet<>(List.of("VEGETARIAN_FRIENDLY")));

        Person person = new Person(); person.setName("Dan");
        person.setDietaryTags(new HashSet<>(List.of(DietaryTag.VEGAN)));

        when(mealRepository.findById(12L)).thenReturn(Optional.of(meal));
        ...
        when(signupRepository.findByPersonIdAndMealDateRange(eq(22L), any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(List.of());
    ...
    void createSignup_successful() {
        Meal meal = new Meal(); meal.setDate(LocalDateTime.now().plusDays(5)); meal.setMaxAttendees(10);
        Person person = new Person(); person.setName("Eve");

        when(mealRepository.findById(13L)).thenReturn(Optional.of(meal));
        ...
        when(signupRepository.findByPersonIdAndMealDateRange(eq(23L), any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(List.of());
